...
- (NSInteger)numberOfRowsInTableView:(NSTableView *)aTableView {
    return (NSInteger)[self.songData count];
}
...
- (NSArray *)chunkArray:(NSArray *)array size:(NSUInteger)size callback:(NSArray *(^)(NSArray *chunk))block {
...
- (IBAction)fetchLibrary:(id)sender {
    ...
    if (!self.updatingLibrary) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            ...
            for (NSUInteger x = 0; x < [fileList count]; x++) {
                if (self.doCancel)
                    return;
                ...
                NSUInteger trackId = [cache trackIdForDatabaseId:(NSUInteger)[track databaseID]];
                ...
                if (trackId == 0) {
                    ...
                    if (trackId > 0) {
                        [cache addTrackId:trackId toDatabaseId:(NSUInteger)[track databaseID]];
                    }
                }
		  ...
		  (unsigned long)[self.songData count]);
            ...
            for (RowData *item in self.songData) {
                NSDictionary *trackInfo = [cache trackInfo:(NSInteger)item.trackId];
                if (trackInfo == nil) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            ...
            self.progressBar.maxValue = [self.songData count];
            for (NSUInteger z = 0; z < [self.songData count]; z++) {
                if (self.doCancel)
                    return;
                dispatch_async(dispatch_get_main_queue(), ^(void) {
